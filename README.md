...
__p.s:__ обратите внимание на то, как в конечном итоге будет выглядит код для вашей ос,
  этим самым я стараюсь вас дисциплировать и писать код красиво, приближенно к окружению unix
  (в следущей части уже будет реализовано в полном объеме) во первых просто для того,
  если вы знакомы с окружением си и unix вам было легко вникуть, а второе это лучше чем в некоторых
  туториалах (которых почти нет) по создагию своих ос, наименование функций имеет подобный вид CreateThread(THREAD_SIZE, INPUT_PARAM ... )
  с огромным количествам параметров, что в итоге код становиться не красивым (хотя именно так реализовано всё в windows :p,
  выглядит и писать системные программы на си под неё просто ужасно)
```c
 #include <stdio.h>
 #include <stdlib.h>
 #include <thread.h>
